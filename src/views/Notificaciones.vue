    ...
    <div class="historial-notificaciones">
      ...
      <div class="notificaciones-container">
        ...
        >
          ...
          <p v-if="!isRepetitive(notificacion)">
            <strong>Estado:</strong> {{ notificacion.estado }}
          </p>
          <div v-else>
            <p><strong>Fecha fin:</strong> {{ formatearFecha(notificacion.fecha_fin) }}</p>
            <p>
              <strong>Intervalo:</strong> {{ notificacion.intervalo }}
              {{ notificacion.unidad_intervalo }}
            </p>
  ...
  methods: {
    ...
    async cargarNotificaciones() {
      ...
      try {
        ...
        const response = await axios.get('http://127.0.0.1:5000/notificacion/notificaciones', {
          params
        })
    ...
    async updateNotification(updatedNotificacion) {
      try {
        ...
        await axios.put(
          `http://127.0.0.1:5000/notificaciones/${updatedNotificacion.id}`,
          updatedNotificacion
        )
...
.footer {
  ...
  text-align: center;
  color: white;
  margin-top: 200px; /* Empuja el footer hacia la parte inferior */
}
